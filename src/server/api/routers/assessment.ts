...
export const assessmentRouter = createTRPCRouter({
  getStatus: protectedProcedure.query(async ({ ctx }) => {
    ...
    return {
      isCompleted:
        !!user?.assessmentCompletedAt || (hasEssentialData && hasEnhancedData),
      hasPartialData: !!(
        user?.relationshipStatus || user?.relationshipGoals?.length
      ),
      data: user,
    };
  }),
  ...
  updateStep: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.step === "relationship-history") {
        if (input.data.relationshipStatus)
          updateData.relationshipStatus = input.data.relationshipStatus;
        if (
          input.data.hasSignificantPast !== undefined ||
      ...
      if (input.step === "emotional-intelligence") {
        if (input.data.relationshipGoals)
          updateData.relationshipGoals = input.data.relationshipGoals;
        if (input.data.relationshipReadiness)
          updateData.relationshipReadiness = input.data.relationshipReadiness;
        if (
          input.data.attachmentStyle ||
      ...
      if (input.step === "values-vision") {
        if (input.data.coreValues)
          updateData.coreValues = input.data.coreValues;
        if (input.data.relationshipVision)
          updateData.relationshipVision = input.data.relationshipVision;
        if (input.data.dealBreakers)
          updateData.dealBreakers = input.data.dealBreakers;
      }

      if (input.step === "lifestyle-compatibility") {
        if (input.data.preferredCommunicationStyle)
          updateData.preferredCommunicationStyle =
            input.data.preferredCommunicationStyle;
        if (
          input.data.workLifeBalance ||
  ...
  repairAssessmentCompletion: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    return {
      repaired: false,
      reason: "User either lacks data or already has completion timestamp",
    };
  }),
